...
Along with harmful BRCA mutations, many other highly penetrant mutations\footnote{most people carrying the mutation will develop the disease} are associated with diseases, and are referenced in an online database \cite[]{hamosh2005online}. 
Those mutations are often very rare and are either associated with some very rare disease or are explaining only a small proportion of common disease incidence.
...
[PARLER DE QQ-PLOT ET DE LAMBDAGC??]
...
First GWAS found only 12 associated SNPs for type 2 diabetes and only 2 for prostate cancer, explaining a very small part of heritability of these diseases \cite[]{jakobsdottir2009interpretation}. 
...
The most widely used method called ``Clumping + Thresholding'' (C+T) has been described in section \ref{sec:C+T}.
...
Our second paper has been dedicated to developing penalized regressions as a replacement to more simple, less optimal methods, and that could be used for large-scale datasets as well.
%As a surprise, we found out that statistical learning methods were useful when sample size was large enough or for autoimmune di
